...
- (Req-ConnectionIntegrity) When Provisioning process requires multiple data exchanges (as described in Req-RoundTrips), no third party shall be allowed to interfere between Sender and Receiver. The solution shall provide integrity to the connection between Sender and Receiver for the duration of the transfer. The tranfer ends when either Sender or Receiver ends it.
...
- (Req-Retrievals) Sender should be able to specify the max number of participant devices that can participate in the credential transfer.
