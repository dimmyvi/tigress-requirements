...
- Depending on credential type, there are 3 possible scenarios for transferred credential cryptographic key material:
  ...
  3) The receiver device generates has a public / private key pair and sends the public key to the owner to be signed. This public key + signature is then the transfered credential.
- Sender device has the means to create or obtain the Provisioning information that is being sent to the receiver.
- The receiver device has the means to turn the Provisioning information into something that is usible. 
...
- The interaction between Sender device and Provisioning Entity.
- The interaction between Receiver device and Provisioning Entity.
